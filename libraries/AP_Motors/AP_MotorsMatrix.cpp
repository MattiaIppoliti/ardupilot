    ...
    switch (_spool_state) {
        ...
        case SpoolState::SPOOLING_DOWN:
            ...
            for (i=0; i< AP_MOTORS_MAX_NUM_MOTORS ; i++) {
                ...
                if(_motor_fail && motor_enabled[i] && (i== _motor_fail_number-1) && (_motor_fail_number > 0) && (_motor_fail_number < AP_MOTORS_MAX_NUM_MOTORS) && (_motor_fail_percent > 0) ) {
                    pwm_fault= output_to_pwm(_actuator[i]);
                    pwm_fault=  pwm_fault - (pwm_fault - get_pwm_output_min()) * _motor_fail_percent / 100.0f;
                    rc_write(i, pwm_fault);
                }
